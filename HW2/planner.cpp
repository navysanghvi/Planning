	   ...
	   int* planlength)
{
  ...
  int lengthPlan; double** finalPlan;
  double extR = 1.0, extS = 0.1, goalB = 0.05;
  int numSamples = 5000;
  
  Rrt* planObject = new Rrt(armstart_anglesV_rad, armgoal_anglesV_rad, numofDOFs,
    x_size, y_size, map, extR, extS, numSamples, goalB);
  tie(lengthPlan, finalPlan) = planObject->RunPlan();
  
  *plan = finalPlan;
  *planlength = lengthPlan;

  cout << lengthPlan << ',' << planObject->m_samplesUsed << ','  
  << planObject->m_samplesAdded << ',' << planObject->m_planTime << ',' 
  << planObject->m_pathQuality << '\n';
     ...
     int* planlength)
{
  ...
  *planlength = 0;
  ...
  RrtConnect* planObject = new RrtConnect(armstart_anglesV_rad, armgoal_anglesV_rad, numofDOFs,
    x_size, y_size, map, extR, extS, numSamples, goalB);
  tie(lengthPlan, finalPlan) = planObject->RunPlan();
     ...
     int* planlength)
{
  ...
  *planlength = 0;
  ...
  int lengthPlan; double** finalPlan;
  double extR = 1.0, extS = 0.1, goalB = 0.05;
  int numSamples = 5000; double nRad = 2;
  
  RrtStar* planObject = new RrtStar(armstart_anglesV_rad, armgoal_anglesV_rad, numofDOFs,
    x_size, y_size, map, extR, extS, numSamples, goalB, nRad);
  tie(lengthPlan, finalPlan) = planObject->RunPlan();
     ...
     int* planlength)
{
  ...
  *planlength = 0;
  ...
  int lengthPlan; double** finalPlan;
  double extS = 0.1; int k = 6; 
  int numSamples = 8000; double nRad = 2;

  PRMap* planObject = new PRMap(armstart_anglesV_rad, armgoal_anglesV_rad, numofDOFs,
    x_size, y_size, map, extS, numSamples, nRad, k);
  tie(lengthPlan, finalPlan) = planObject->RunPlan();
     ...
     int* planlength)
{
  ...
  *planlength = 0;
  ...
  for(int i = 0; i < 40; i++)
  {
    do{ qValid = planObject->GetSample(); } while(!(planObject->IsValidArmConfiguration(qValid)));
    samples.push_back(qValid);
  }

  tie(lengthPlan, finalPlan) = planObject->GetFinalPlan(samples,0);
  *plan = finalPlan;
  *planlength = lengthPlan;
